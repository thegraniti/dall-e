...
import { getRandomPrompt } from "../utls";
...
function CreatePost() {
  ...
  async function handleSubmit(event) {
    ...
    if(form.prompt && form.photo){
      setLoading(true);
      try{
          const response = await fetch('https://dall-e-qttf.onrender.com/api/v1/post',
          {
            method: 'POST',
            headers: {
  ...
  async function generateImage() {
      ...
      if(form.prompt){
        try {
          setGenerateImg(true)          
          const response = await fetch('https://dall-e-qttf.onrender.com/api/v1/dalle', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
